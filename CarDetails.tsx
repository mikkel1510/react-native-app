import { useRoute, useNavigation } from "@react-navigation/native"; // added navigation so we can pass data to Home
...
const CarDetailsScreen: React.FC = () => {
    const navigation = useNavigation(); // used to send rental data to Home
    ...
    const toggleRented = () => {
        const next = !isRented; // check next rent state
        setRented(next);
        setPopUpVisible(!isPopUpVisible);

        // send info to Home when a car is rented
        if (next) {
            const endsAt = new Date(Date.now() + 2 * 60 * 60 * 1000).toISOString(); // +2h just for demo

            // navigate back to Home with activeRental data
                ...
                <View style={styles.popup}>
                    ...
                    >
                        ...
                        <View style={{ paddingTop: 50 }}>
                            <View
                                style={[styles.popupRow, { justifyContent: "space-between" }]}
                            >
                                <Text>Period</Text>
                                <Text>Enddate</Text>
                            </View>
                    ...
                    <View style={styles.popupRow}>
                        {/* confirm also updates Home to show current rental */}
                        <Pressable
                            style={[styles.button, { backgroundColor: Colors.confirm }]}
                            onPress={toggleRented}
                        >
